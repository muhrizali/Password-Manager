...
def fn_masterpass(n, d, m):  # This function generates a masterpassword unique to every user
    ...
    password = password1[0:2] + sp_for_pass[0] + password1[2:4] + sp_for_pass[1] + password1[4:6] + sp_for_pass[2] + password1[6:]
...
def fn_make_user_data(n, d, m, p):  # This function creates dictionary data of the user
    # that we need to store in binary.
...
def fn_dump_user_data(user_data, saved_data_list):  # This function saves the new user data to
    # the binary file.
...
def fn_get_users_data():  # This function gets all the users data from binary and make it into a list.
...
def fn_dump_generated_pass(resp, account_name, selected_user_data, list_of_data):
    ...
    # user data is saved into the binary file.
...
def fn_display_user_passwords(selected_user_data, showpass = False):  # This function displays all the passwords of the logged in user.
...
def fn_new_user(list_of_data):
    ...
    while not userExists:
        ...
        if userExists:
            print()
            fn_animation("A user with name " + name + " already exists.")
            print()
            print("  @ Try with another name :")
            print()
            print("  @ Or type 'logout' to log yourself out :")
...
def fn_existing_user(selected_user_data):  # This function deals with every existing user.
    ...
    while not correct_pin:
        ...
        print()
        print("\n" * 80)
        fn_animation("Incorrect Pin : Try Again")
        print()
        print("   @ Or type 'logout' to log yourself out:")
        print()
...
def fn_create_new_pass(resp, account_name, selected_user_data, list_of_data):
    ...
    updated_list_of_data = fn_dump_generated_pass(resp, account_name, selected_user_data, list_of_data)
    return updated_list_of_data
...
def fn_password_exists(account_name, selected_user_data):
    ...
    for old_acc_name in list_of_accounts:
        if old_acc_name == account_name:
            pass_exists = True
            print("\n" * 80)
            fn_animation("Account Already Exists")
            print()
            print()
...
def fn_update_pass(account_index, selected_user_data, list_of_data):
    ...
    while True:
        ...
        if password_matches:
            ...
            break
    fn_dialogue("Updating " + account_name) 
...
def fn_delete_pass(account_index, selected_user_data, list_of_data):
    ...
    account_list = list(selected_user_data["passwords"].keys())
    account_name = account_list[account_index]
    fn_dialogue("Deleting " + account_name)
...
def fn_animation(message):  # This is just an animation which pops at certain occasions. 
    char = "**"
    print()
    print()
...
def fn_savetext(selected_user_data):
    ...
    for old_account, passw in (selected_user_data["passwords"]).items():
        if old_account == "masterpass":
            continue
        file_obj.write("      " + str(index) + " ~ " + old_account + " : " + passw + "\n\n")
        index = index + 1
...
while True:
    ...
    print()
    print("    @ Type 'newuser' to create a new account :\n\n")
    resp = input("    : ")
    ...
    if resp.lower() == "newuser":
        ...
        fn_dialogue("Clearing All Users Data")
        file_obj = open("usersdata.bin", "wb")
        file_obj.close()
        continue
    
    elif resp.lower() == "--pinky":
        fn_dialogue("Getting All Users Data")
        fn_pinky(listOfData)
        continue
    ...
    while LoggedIn:
        ...
        if first_run:
            fn_dialogue("Logging in as " + selectedUserData["username"])
            first_run = False
        ...
        if "newpass" in sec_resp.lower():
            ...
            while run:
                ...
                while not account_valid:
                    print()
                    print("   @ Type the account for the password : (Google, Netflix, Amazon, Instagram...)")
                    print()
                    account = input("\t : ")
                ...
                while not username_valid:
                    print()
                    print("   @ Type the username for the account : (Muhriz Ali, Julie_M , poonam236, PrinceKS...)")
                    print()
                    username = input("\t : ")
                    print()
                ...
                accountExists = fn_password_exists(accountName, selectedUserData)
                if not accountExists:
                    listOfData = fn_create_new_pass(sec_resp, accountName, selectedUserData, listOfData)
                    run = False
            ...
            if updateAccountIndex >= len(accountList) or updateAccountIndex <= 0:
                fn_animation("Account does not exist.")
                continue
            listOfData = fn_update_pass(updateAccountIndex, selectedUserData, listOfData)
            ...
            if deleteAccountIndex >= len(accountList) or deleteAccountIndex <= 0:
                fn_animation("Account does not exist.")
                continue
            listOfData = fn_delete_pass(deleteAccountIndex, selectedUserData, listOfData)
            ...
            fn_dialogue("Saving Passwords as Text")
